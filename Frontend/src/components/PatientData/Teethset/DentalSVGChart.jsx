...
// ============================================
const canAccessDentalChart = () => {
    if (!subscriptionData) return true;
    return subscriptionData.planType !== 'Basic Plan';
};
...
const handleToothClick = (toothId, e) => {
    ...
    if (!canAccessDentalChart()) {
        navigate('/pricing');
        return;
    }
    // Your existing tooth click logic
...
<div className="relative">
    {/* Removed the popup modal - direct navigation to pricing page */}

    <div className="relative bg-white group">
        {/* Initial blur overlay for Basic Plan users - visible by default */}
        {!canAccessDentalChart() && (
            <div className="absolute inset-0 z-10 bg-gray-900/10 backdrop-blur-xs rounded-lg"></div>
        )}
        
        {/* Hover Overlay for Basic Plan users - shows on hover */}
        {!canAccessDentalChart() && (
            <div 
                className="absolute inset-0 z-20 bg-gray-900/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center rounded-lg cursor-pointer"
                onClick={() => navigate('/pricing')}
            >
                <div className="text-center text-white px-6 pointer-events-none items-center">
                    <div className="mb-3">
                        <svg className="w-16 h-16 mx-auto text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
                        </svg>
                    </div>
                    <p className="text-xl font-bold mb-2">Upgrade Required</p>
                    <p className="text-sm opacity-90 underline text-[#667EEA]">view pricing</p>
                </div>
            </div>
        )}
                ...
                return (
                    <g key={toothId} className="tooth-element">
                        <path
                            ...
                            className={`${!canAccessDentalChart() ? 'cursor-not-allowed' : 'cursor-pointer'} transition-all duration-300 ${
                                isCommentMode 
